  ...
  try {
    ...
    if (action === "latest" || action === "completed" || action === "popular") {
      ...
      if (action === "popular") {
        ...
        url2 = `https://nhentai.net/search/?q=""&sort=${popularSort}&page=${page}`;
      } else {
        url2 = `https://nhentai.net/?page=${page}`;
      }
      const resp = await axios_default.get(url2, { headers });
    ...
    if (action === "search") {
      ...
      const resp = await axios_default.get(queryUrl.toString(), { headers });
      const $ = cheerio.load(resp.data);
      const items = [];
      $(".gallery").each((_, el) => {
        const id = $(el).find("a").attr("href")?.split("/")[2];
    ...
    if (action === "detail") {
      ...
      const url2 = `https://nhentai.net/g/${slug}/`;
      const resp = await axios_default.get(url2, { headers });
      const $ = cheerio.load(resp.data);
