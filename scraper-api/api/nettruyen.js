...
var cachedDomainExpiry = 0;
var getOrderedDomains = () => {
...
var setCachedDomain = (domain) => {
  cachedDomain = domain;
  cachedDomainExpiry = Date.now() + 30 * 60 * 1e3;
};
var UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36";
var fetchPage = async (path, headers) => {
  const domains = getOrderedDomains();
  let lastError = null;
  for (const domain of domains) {
    try {
      const url2 = path.startsWith("http") ? path : `https://${domain}${path}`;
...
var makeAbsoluteUrl = (url2, domain) => {
  if (!url2) return "";
  url2 = url2.trim();
  const domainPrefix = `https://${domain}/https`;
  if (url2.startsWith(domainPrefix)) {
    url2 = url2.substring(`https://${domain}/`.length);
  } else if (url2.startsWith(`http://${domain}/https`)) {
    url2 = url2.substring(`http://${domain}/`.length);
  }
  if (/^\/+https?:/.test(url2)) {
    url2 = url2.replace(/^\/+/, "");
  }
  ...
  if (url2.startsWith("/")) return `https://${domain}${url2}`;
  return `https://${domain}/${url2}`;
};
var extractSlug = (href) => {
  const match = href.match(/\/(?:manga|truyen-tranh)\/([^/?#]+)/);
  return match ? match[1] : "";
};
var extractChapterSlug = (href) => {
  const match = href.match(/\/(?:manga|truyen-tranh)\/[^/]+\/([^/?#]+)/);
  return match ? match[1] : "";
};
var parseItemList = (html, domain, pageStr) => {
