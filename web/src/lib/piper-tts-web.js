...
const HF_BASE = "https://raw.githubusercontent.com/laoto1/TTS/main/voices";
...
async function writeBlob(url, blob) {
  if (!url.match("https://raw.githubusercontent.com/")) return;
  try {
...
const _TtsSession = class _TtsSession {
  ...
  async init() {
    ...
    __privateGet(this, _ort).env.wasm.proxy = true; // Use Web Worker to prevent UI blocking
    __privateGet(this, _ort).env.wasm.wasmPaths = __privateGet(this, _wasmPaths).onnxWasm;
    const path = PATH_MAP[this.voiceId] || `vi/${this.voiceId}.onnx`;
  ...
  async predict(text, options = {}) {
    ...
    const noiseScale = options.noiseScale ?? __privateGet(this, _modelConfig).inference.noise_scale;
    const lengthScale = options.lengthScale ?? __privateGet(this, _modelConfig).inference.length_scale;
    const noiseW = options.noiseW ?? __privateGet(this, _modelConfig).inference.noise_w;
...
async function download(voiceId, callback) {
  const path = PATH_MAP[voiceId] || `vi/${voiceId}.onnx`;
