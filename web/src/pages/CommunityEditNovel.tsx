...
const CommunityEditNovel = () => {
    ...
    useEffect(() => {
        if (!user || !token) {
            toast.error('Bạn cần đăng nhập để sửa truyện!');
            navigate(`/login?redirect=/community/novel/${id}/edit`);
        }
    ...
    useEffect(() => {
        if (novelData) {
            // Verify ownership
            if (String(user?.id) !== String(novelData.owner_id)) {
                toast.error('Bạn không có quyền sửa truyện này!');
                navigate(-1);
                return;
            }

            setTitle(novelData.name);
            setDescription(novelData.description);
            setCoverUrl(novelData.thumb_url);
            setStatus(novelData.status);
            setSelectedCategories(novelData.categories.map((c: any) => c.name));
        }
    }, [novelData, user, navigate]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await axios.put(`https://share.laoto.workers.dev/api/community/novels/${id}`, {
                title: title.trim(),
            ...
            if (res.data.status === 'success') {
                ...
                // Invalidate query to refetch details
                queryClient.invalidateQueries({ queryKey: ['communityNovel', id] });
                queryClient.invalidateQueries({ queryKey: ['communityNovels'] });
                navigate(`/community/novel/${id}`);
            } else {
                toast.error(res.data.message || 'Lỗi khi sửa truyện!');
            }
        } catch (error: any) {
            console.error(error);
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[var(--color-bg)]">
            <header className="sticky top-0 z-40 bg-[var(--color-surface)] flex items-center h-14 px-4 shadow-sm border-b border-[var(--color-border)]">
                ...
                <h1 className="text-lg font-medium ml-2 flex-1">Sửa truyện</h1>
                <button 
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <select 
                            ...
                            className="w-full px-4 py-3 bg-[#1e1e1e] border border-[#333] rounded-xl text-white outline-none focus:border-[var(--color-primary)] transition-colors appearance-none"
                        >
...
export default CommunityEditNovel;
