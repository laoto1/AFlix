                    ...
                    <div className="flex overflow-x-auto no-scrollbar gap-2 px-4 pb-3 border-t border-[var(--color-border)] pt-3 bg-[var(--color-surface)]">
                        {(activeTab === 'filters' ? ThePYService.FILTER_FIELDS : ThePYService.SORT_FIELDS).map(c => (
                            <button
                                ...
                                className={`shrink-0 px-3 py-1 rounded-full text-[11px] font-medium transition-colors border ${sortField === c.value || (!sortField && activeTab === 'filters' && c.value === ThePYService.FILTER_FIELDS[0]?.value) || (!sortField && (activeTab === 'recent' || activeTab === 'phim-moi-cap-nhat') && c.value === 'recent')
