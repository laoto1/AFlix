Method: GET
URL
http://14.225.254.182/stv.ui.js?v=1.283
Headers
Accept:
*/*
Accept-Encoding:
...
function ajax(params, callb, retry) {
  ...
  if (params.indexOf("ajax=") >= 0) {
    url += "?ngmar=" + params.substr(params.indexOf("ajax=") + 5, 8);
  }
  if (params && params.substring(0, 4) == "/io/") {
    url = params;
    params = "";
  }
  ...
  http.onreadystatechange = function() {
    if (http.readyState == 4 && http.status == 200) {
      if (callb != null) callb(this.responseText, this.getAllResponseHeaders());
...
ui.zoomText = function(e, subSelector = null, saveScaleId = null, forceAllowZoom = false) {
  var viewportMeta = document.querySelector('meta[name="viewport"]');
  var isZoomable = viewportMeta.getAttribute('content').indexOf('user-scalable=yes') !== -1;
  if (!isZoomable || forceAllowZoom) {
    if (saveScaleId) {
      saveScaleId = "ui_scale_" + saveScaleId;
      var scale = localStorage.getItem(saveScaleId);
      if (scale) {
        if (subSelector) {
          var sub = e.querySelectorAll(subSelector);
          for (var i = 0; i < sub.length; i++) {
            sub[i].style.fontSize = scale + 'px';
          }
        } else {
          e.style.fontSize = scale + 'px';
        }
      }
    }
    var getGetSetSizeElement = function() {
      if (subSelector) {
        return e.querySelector(subSelector) || e;
      }
      return e;
    }
    var touch1, touch2, distance, scale, lastScale = 1,
      initialFontSize = parseFloat(window.getComputedStyle(getGetSetSizeElement()).fontSize);
    var currentFontSize = initialFontSize;
    var getAllTouches = function(ev) {
      var l = ev.touches || ev.changedTouches;
      return l;
    }
    var ontouchstart = function(ev) {
      var ts = getAllTouches(ev);
      if (ts.length === 2) {
        touch1 = ts[0];
        touch2 = ts[1];
        distance = Math.sqrt(Math.pow(touch1.clientX - touch2.clientX, 2) + Math.pow(touch1.clientY - touch2.clientY, 2)) / 5; // divide by 5 to make it less sensitive
        initialFontSize = parseFloat(window.getComputedStyle(getGetSetSizeElement()).fontSize);
      }
      e.addEventListener('touchmove', ontouchmove);
    ...
    var ontouchmove = function(ev) {
      ...
      if (ts.length === 2) {
        touch1 = ts[0];
        touch2 = ts[1];
        var newDistance = Math.sqrt(Math.pow(touch1.clientX - touch2.clientX, 2) + Math.pow(touch1.clientY - touch2.clientY, 2)) / 5; // divide by 5 to make it less sensitive
        scale = newDistance / distance;
        var newFontSize = Math.round(initialFontSize * scale);
        if (newFontSize !== currentFontSize && newFontSize > 8 && newFontSize < 50) {
          //e.style.fontSize = newFontSize + 'px';
          if (subSelector) {
            var sub = e.querySelectorAll(subSelector);
            for (var i = 0; i < sub.length; i++) {
              sub[i].style.fontSize = newFontSize + 'px';
            }
          } else {
            e.style.fontSize = newFontSize + 'px';
          }
          ...
          if (saveScaleId) {
            localStorage.setItem(saveScaleId, newFontSize);
          }
        }
        lastScale = scale;
        if (forceAllowZoom) {
...
ui.stab = function(tabber, tabdiv) {
  ...
  for (var i = 0; i < tabber.children.length; i++) {
      ...
      function() {
        for (var j = 0; j < tabber.children.length; j++) {
          if (tabber.children[j] == this) {
            ...
            if (this.hasAttribute("onfirstload")) {
              var firstloadHandler = this.getAttribute("onfirstload");
              this.removeAttribute("onfirstload");
              eval(firstloadHandler);
            }
          } else {
            tabber.children[j].classList.remove("active");
          }
    ...
    tabdiv.children[i].style.width = widthspl + "%";
  }
...
ui.smtab = function(tab, snapfirst, nodrag, noswipe, animateMethod, useCvRender) {
  ...
  var isNoMark = false;
  if (tabpointer) {
    if (tabpointer.style.display == "none") {
      isNoMark = true;
    }
  }
      ...
      function() {
        for (var j = 0; j < tabber.children.length; j++) {
          if (tabber.children[j] == this) {
            ...
            if (animateMethod == 0 || animateMethod == "smooth") {
              $(tabdivcontainer).animate({
                scrollLeft: nextPos
              }, timeneed, "swing", function() {
                tabdivcontainer.isanimating = false;
                if (animateMethod == "smooth") {
                  tabdivcontainer.style.overflowX = "scroll";
                }
              });
              ...
              gsap.fromTo(tabdiv, {
                x: getPxFromTranslate(tabdiv.style.transform)
              }, {
                x: -nextPos,
                duration: timeneed / 1000,
                ease: "Power2.easeOut",
                onComplete: function() {
                  tabdivcontainer.isanimating = false;
                }
            ...
            if (isNoMark) {} else if (useCvRender) {
  ...
  if (tab.querySelector("ntab")) {
    ...
    tabber.addEventListener("scroll", function() {
      ntab.parentElement.scrollLeft = tabber.scrollLeft
    });
  ...
  if (animateMethod === "smooth") {
    tabdivcontainer.classList.add("scrollx");
    // var childCount = tabdiv.children.length;
    // for(var i=0;i<childCount;i++){
    // 	let ttabview = tabdiv.children[0];
    // 	ttabview.classList.add("scrollx");
    // 	tabdivcontainer.appendChild(ttabview);
    // }
    // tabdiv.remove();
    // tabdiv = tabdivcontainer;
  ...
  if (!noswipe && animateMethod !== 1) {
    ui.swipe(tabdivcontainer, "left", function() {
      if (animateMethod == "smooth") {
        tabdivcontainer.style.overflowX = 'hidden';
      }
      tabdiv.nextTab();
    }).setThreshold(50);
  ...
  function touchstart() {
    ...
    if (tc0) {
      ...
      if (!window.lockDrag) {
        ...
        firstTouch = true;
        tabdivcontainer.addEventListener("touchmove", touchmove, {
          passive: false
        });
      }
      //},30);
  ...
  tabdiv.style.transform = 'translateZ(0px)';
    ...
    if (tc0) {
      if (firstTouch) {
        ...
        if (Math.abs(offsetX) < Math.abs(offsetY)) {
          dragstart = false;
          tabdivcontainer.removeEventListener("touchmove", touchmove);
          return;
        }
        tempDisableObserver();
      }
  ...
  function snap() {
    var mindist = 9999,
      mini = ctab,
      half = (snapPoint[1] - snapPoint[0]) / 2,
      cp = tabdivcontainer.scrollLeft + half; //getInt(tabdiv.style.transform) +half;//
  ...
  if (!nodrag && animateMethod !== 1 && animateMethod != "smooth") {
  ...
  if (!nodrag && animateMethod == "smooth") {
    tabdivcontainer.addEventListener("touchstart", touchstart);
    tabdivcontainer.addEventListener("touchend", function() {
      tabdivcontainer.style.overflowX = 'hidden';
      // tabdivcontainer.scrollTo({
      ...
      // });
      if (tabdivcontainer["sw-left"].timer == 0 || tabdivcontainer["sw-left"].outoftime || window.tabdragging != tab.id) {
        if (!tabdivcontainer.isanimating) snap();
      }
      dragstart = false;
    });
    tab.addEventListener("touchcancel", function() {
  ...
  tab.setActiveStyle = function() {
    for (var i = 0; i < tabber.children.length; i++) {
      if (i == ctab) {
        tabber.children[i].classList.add("active");
      } else {
        tabber.children[i].classList.remove("active");
      }
    }
  }
...
ui.filterSel = function(input, container, sel) {
  ...
  container.qq("[sleepwake]").forEach(function(e) {
    e.wake()
  });
}
...
function loadConfig() {
  if (!window.localStorage) {
    window.localStorage = {
      ...
      getItem: function(key) {
        return this.tempMemory[key] || null;
      },
      setItem: function(key, value) {
        this.tempMemory[key] = value;
      },
      removeItem: function(key) {
        if (this.tempMemory[key]) {
    ...
    Object.defineProperty(window.localStorage, 'length', {
      get: function() {
        return Object.keys(this.tempMemory).length;
      }
    });
  }
  ...
  if (setting != null) {
    ...
    try {
      ...
      ui.notif("Không thể lưu cấu hình đọc truyện vào bộ nhớ trình duyệt, có thể do bộ nhớ đã đầy hoặc trình duyệt của bạn chặn lưu trữ cục bộ.");
    }
  }
  ...
  document.querySelectorAll(".textzoom").forEach(function(e) {
    try {
      ui.zoomText(e, e.getAttribute("zoom-sel"), e.getAttribute("zoom-save"), e.getAttribute("zoom-force"));
    } catch (ex) {}
  });
  //synctusach();
  //syncvp();
...
function notification() {
  ...
  xhttp.open("POST", "/io/user/notification", true);
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var page = 0;
      var c = md.body();
      ui.infinity(c, /*"ajax=notification"*/ function () {
        page += 1;
        c.ended = true;
        ajaxUrl("/io/user/notification", "page=" + page, function (d) {
            c.ended = d.indexOf("class='noti'") < 0;
            c.onautoload(d);
        });
...
ui.style.parsecss = function (txt) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        if (_t != "") {
            ...
            if (_t[0] != "-")
                css[express[0].replace(/([a-z]\-[a-z])/g, function (a, b) {
                    return a[0] + a[2].toUpperCase()
                })] = express[1].trim() || "";
        }
    }
    return css;
...
function clearOldHistory() {
    ...
    if ([].filter) {
        var cleaned = ts.filter(function (e) {
            return e != "";
        }).join("~/~");
...
ui.dragOut = function (e, c) {
    ...
    var tmove = function (e) {
        ...
        if (!isCancel) {
            ...
            if (co != 0) {
                ...
                e.stopPropagation();
                var newTransform = "translate3d(" + co + "px,0,0)";
    ...
    e.addEventListener("touchstart", function (e) {
        ...
        this.addEventListener("touchmove", tmove, {
            passive: false
        });
...
ui.dragOutScroll = function (e, c) {
    ...
    var isTouchEnd = false,
        onScrollEnd = null;
    ui.scriptmanager.load("https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/ScrollToPlugin.min.js", function () {
    ...
    var css = [
        "width: 100vw;height: 100vh;margin-left: 100vw;margin-right: 100vw;box-shadow: 0 0 20px 0px black;will-change:initial;position:relative;contain:initial", // for container
        "overflow: scroll;background: transparent !important; background-color:transparent !important", // for body
        "background: transparent !important; background-color:transparent !important", // for html,body
    ];
    var vw = document.body.scrollWidth;
    var backToCenter = function () {
        if (!isScrollEnd) {
    ...
    var toDirection = function (dir, cb) {
        if (!isScrollEnd) {
            onScrollEnd = function () {
                toDirection(dir, cb);
                onScrollEnd = null;
        ...
        c.style.pointerEvents = "none"; // disable pointer events
        var left = e.parentElement.scrollLeft;
        var vw = document.body.scrollWidth;
        if (dir) { // ltr
            var dur = 0.25 * Math.abs(left / vw);
            gsap.to(e.parentElement, {
                scrollTo: {
                    x: 0,
                    autoKill: false
            ...
            var dur = 0.25 * Math.abs((vw * 2 - left) / vw);
            gsap.to(e.parentElement, {
                scrollTo: {
    ...
    e.parentElement.addEventListener("touchstart", function (ev) {
        if (ev.target && ev.target.hasAttribute("type")) {
            if (ev.target.getAttribute("type") == "range") {
                isCancel = true;
                return;
            }
        }
    ...
    e.parentElement.addEventListener("touchend", function (ev) {
        initDrag = false;
        isTouchEnd = true;
        var cx = ev.changedTouches[0].screenX,
            cy = ev.changedTouches[0].screenY;
        co = cx - sx;
        if (Math.abs(cy - sy) > Math.abs(co)) {
            isSwipeTimeout = true;
        }
        if (isCancel) {
            if (co != 0) {
    ...
    e.parentElement.addEventListener("touchcancel", function () {
        isCancel = true;
        isSwipeTimeout = true;
        clearTimeout(timer);
        if (co != 0) {
    ...
    e.parentElement.addEventListener("scroll", function () {
        var t = Date.now();
    ...
    e.setAttribute("style", css[0]);
    e.parentElement.setAttribute("style", css[1]);
    //document.documentElement.setAttribute("style", css[2]);
    e.parentElement.parentElement.setAttribute("style", css[2]);
    e.parentElement.scrollLeft = vw;
    c.setAttribute("style", css[2]);
...
ui.snapY = function (target, snapPoint, onSnap) {
    ...
    var dragStartFunction = function (e) {
        sy = e.touches[0].clientY;
        target.addEventListener("touchmove", dragFunction, {
            passive: false
        });
    }
...
function activeunity() {
    window.setting.allowunitymode = true;
    store.setItem("setting", JSON.stringify(setting));
    store.setItem("unitymode", "true");
    location.reload();
}
...
ui.captcha = {
    google: {
        load: function () {
            var scriptUrl = "https://www.google.com/recaptcha/api.js?onload=onCaptchaReady&render=explicit";
            var script = document.createElement("script");
            script.src = scriptUrl;
            window.onCaptchaReady = function () {
                ui.captcha.google.onReady();
            }
            document.head.appendChild(script);
        },
        ready: false,
        onReady: function () {
            ui.captcha.google.ready = true;
            if (window.waitForCaptchaReady) {
                window.waitForCaptchaReady();
                window.waitForCaptchaReady = null;
            }
        },
        render: function (id, callback) {
            ui.captcha.google.widget = grecaptcha.render(id, {
                'sitekey': '6LePXXgpAAAAAI6Z-0FWdSCkrknINfR1LvfY1MwK',
        ...
        reset: function () {
            if (ui.captcha.google.widget) {
                grecaptcha.reset(ui.captcha.google.widget);
            }
        },
        remove: function () {
            if (ui.captcha.google.widget) {
                ui.captcha.google.widget = null;
            }
        }
    },
    cloudflare: {
        load: function () {
            var scriptUrl = "https://challenges.cloudflare.com/turnstile/v0/api.js?onload=onCaptchaReady&render=explicit";
            var script = document.createElement("script");
            script.src = scriptUrl;
            script.setAttribute("async", "true");
            window.onCaptchaReady = function () {
                ui.captcha.cloudflare.onReady();
            }
            document.head.appendChild(script);
        },
        ready: false,
        onReady: function () {
            ui.captcha.cloudflare.ready = true;
            if (window.waitForCaptchaReady) {
                window.waitForCaptchaReady();
                window.waitForCaptchaReady = null;
            }
        ...
        render: function (id, callback) {
            ui.captcha.cloudflare.widget = turnstile.render("#" + id, {
                'sitekey': '0x4AAAAAABVjME7NHipdnj-c',
                'callback': function (token) {
                    if (callback) {
                        callback(token);
                    }
        ...
        reset: function () {
            if (ui.captcha.cloudflare.widget) {
                turnstile.reset(ui.captcha.cloudflare.widget);
            }
        },
        remove: function () {
            if (ui.captcha.cloudflare.widget) {
                turnstile.remove(ui.captcha.cloudflare.widget);
            }
        }
    },
    sangtacviet: {
        ...
        ready: true,
        render: function (id, callback) {
            var widget = ui.captcha.sangtacviet.widget = document.createElement("div");
            widget.setAttribute("class", "stv-captcha");
            widget.innerHTML = `
				<div class="captcha-box">
					<div style="padding: 6px"><textarea name="captcha-input" class="captcha-input" placeholder="Nhập mã xác thực" style="resize: none;
					height: 40px;min-height: 40px;text-align: center; border: none; border-radius: 6px;"></textarea></div>
					<div class="captcha-image-box">
						<img class="captcha-image" alt="Captcha">
					</div>
				</div>
				<div style="padding: 4px;">
					<button class="captcha-button w-100">Xác thực</button>
				</div>
				`;
            var input = widget.querySelector(".captcha-input");
            var button = widget.querySelector(".captcha-button");
            var image = widget.querySelector(".captcha-image");
            var refreshCaptcha = function () {
                var imgUrl = "/generate_captcha.php?random=" + Math.random();
                image.src = imgUrl;
                input.value = "";
                input.focus();
            }
            ...
            g(id).appendChild(widget);
            ui.keyboard.showWhenDeviceKeyboardIsNotAvailable(input, document.body);
            input.focus();
        },
        reset: function () {
            if (ui.captcha.sangtacviet.widget) {
                var widget = ui.captcha.sangtacviet.widget;
                var input = widget.querySelector(".captcha-input");
                input.value = "";
                var image = widget.querySelector(".captcha-image");
                var imgUrl = "/generate_captcha.php?random=" + Math.random();
        ...
        remove: function () {
            if (ui.captcha.sangtacviet.widget) {
                ...
                ui.captcha.sangtacviet.widget = null;
            }
        }
    },
    open: function (purpose, provider, onSuccess) {
        if (!provider) {
            provider = "google";
        }
        if (!purpose) {
        ...
        var w = ui.win.create("Captcha");
        w.style.zIndex = "999999";
        var body = w.body();
        var head = w.querySelector(".head");
        var box = w.firstChild;
        head.querySelector(".fuller").remove();
        var providerSelector = document.createElement("span");
        providerSelector.innerHTML = `
			<span class="captcha-provider google"></span>
			<span class="captcha-provider cloudflare"></span>
			<span class="captcha-provider sangtacviet"></span>
		`;
        head.insertBefore(providerSelector, head.querySelector(".closer"));
        box.setAttribute("style", "width: auto; min-height: auto; display: inline-block; min-width: 300px");
        var reNew = function () {
            body.innerHTML = "<div id='captcha'></div>";
        }
        reNew();
        body.style.minHeight = "65px";
        w.show();
        ui.captcha.captchaWindow = w;
        var loadCaptcha = function () {
            var load = function () {
                ui.captcha[provider].remove();
                ui.captcha[provider].render('captcha', function (token) {
                    ui.captcha.onSuccess(token, provider, purpose, onSuccess);
                });
            }
            if (ui.captcha[provider].ready) {
                load();
            } else {
                window.waitForCaptchaReady = function () {
                    load();
                };
                ui.captcha[provider].load();
            }
        }
        loadCaptcha();
        providerSelector.addEventListener("click", function (e) {
            if (e.target && e.target.classList.contains("captcha-provider")) {
                var newProvider = e.target.className.split(" ")[1];
                if (newProvider != provider) {
                    ui.captcha[provider].remove();
                    reNew();
                    provider = newProvider;
                    selectedProvider = newProvider;
                    providerSelector.querySelector(".selected") && providerSelector.querySelector(".selected").classList.remove("selected");
                    e.target.classList.add("selected");
                    loadCaptcha();
                }
            }
        });
        providerSelector.querySelector("." + provider).classList.add("selected");
    },
    onSuccess: function (token, provider, purpose, onSuccess) {
        ajax("ajax=verifycaptcha&token=" + encodeURIComponent(token) + "&purpose=" + purpose + "&provider=" + provider, function (res) {
            if (res == "success") {
                ui.win.close(ui.captcha.captchaWindow);
                if (purpose == "register") {
                    if (onSuccess) {
                        onSuccess();
                    }
...
ui.keyboard = {
    keys: ["1234567890", "qwertyuiop", "asdfghjkl", ["z", "x", "c", "v", "b", "n", "m", "BACKSPACE"]],
    render: function (container, onKeyPress) {
        var kb = document.createElement("div");
        kb.className = "virtual-keyboard";
        ui.keyboard.keys.forEach(function (row) {
            ...
            for (var i = 0; i < row.length; i++) {
                ...
                if (key == " ") {
                    keyDiv.classList.add("vk-space");
                } else if (key == "BACKSPACE") {
                    keyDiv.classList.add("vk-backspace");
                    keyDiv.innerHTML = "⌫";
                } else {
                    keyDiv.innerHTML = key;
                }
                keyDiv.addEventListener("click", function (k) {
                    return function () {
                        onKeyPress(k);
                    }
                }(key));
    ...
    showWhenDeviceKeyboardIsNotAvailable: function (inputElement, container) {
        var isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0;
        if (!isTouchDevice) {
            return;
        }
        var isChrome = 'virtualKeyboard' in navigator;
        if (!isChrome) {
            return;
        }

        inputElement.addEventListener("focus", function () {
            var oldKeyboard = q(".virtual-keyboard")[0];
            if (oldKeyboard) {
                if (oldKeyboard.targetInputElement == inputElement) {
                    return;
                }
                q(".virtual-keyboard").forEach(function (kb) {
                    kb.remove();
                });
            }
            setTimeout(function () {
                if (document.activeElement === inputElement) {
                    ...
                    if (vk.boundingRect.height === 0) {
                        // show our custom keyboard
                        var kb = ui.keyboard.render(container, function (key) {
                            if (key == "BACKSPACE") {
                                ...
                                var end = inputElement.selectionEnd;
                                var value = inputElement.value;
                                inputElement.value = value.substring(0, start) + key + value.substring(end);
                                inputElement.selectionStart = inputElement.selectionEnd = start + key.length;
                            }
                            inputElement.focus();
                        });
                        kb.targetInputElement = inputElement;
                        inputElement._virtualKeyboardElement = kb;
                    }
                }
            }, 500);
        });
        inputElement.addEventListener("blur", function () {
            setTimeout(function () {
                if (inputElement._virtualKeyboardElement && document.activeElement !== inputElement) {
                    inputElement._virtualKeyboardElement.remove();
                    inputElement._virtualKeyboardElement = null;
                }
            }, 300);
        });
    }
...
function loadCaptchaScript() {
    ...
    // if(d!="sangtacviet.com"&&d!="sangtacviet.vip"){
    // 	location.href = location.href.replace(location.hostname, "sangtacviet.vip");
    // 	return;
    // }
    var scriptUrl = "https://www.google.com/recaptcha/api.js?onload=onCaptchaReady&render=explicit";
...
function openCaptcha(p) {
    ...
    if (window.captchaReady) {
        ...
        body.innerHTML = "<div id='captcha'></div>";
        w.show();
        window.captchaWidget = grecaptcha.render('captcha', {
            'sitekey': '6LePXXgpAAAAAI6Z-0FWdSCkrknINfR1LvfY1MwK',
            'callback': function (token) {
                onCaptchaSuccess(token, p);
            }
        });
    } else {
        window.waitForCaptchaReady = function () {
...
function loadComic(i) {
    g(contentcontainer).innerHTML = '<div class="chapcontent" id="chapcontent" style="width: 100%;display: flex; flex-direction: column"></div>';
    ui.scriptmanager.load("/stv.comic.js", function () {
        ui.scriptmanager.load("/stv.comic.web.js", function () {
            translator.getImgList(i);
        }, true);
    }, true);
    var rescale = function () {
        q(".imgblock").forEach(function (img) {
            var renderedWidth = img.offsetWidth;
            if (img.lastScaleW == renderedWidth) {
                return;
            } else {
                img.lastScaleW = renderedWidth;
            }
            var scale = renderedWidth / img.querySelector("img").naturalWidth;
            img.qq(".comic-block").forEach(function (blockElement) {
                var block = blockElement.block;
                blockElement.style.width = block.width * scale + 'px';
                blockElement.style.height = block.height * scale + 'px';
                blockElement.style.left = block.x * scale + 'px';
                blockElement.style.top = block.y * scale + 'px';
                setFitFontSize(blockElement);
            });
            img.rescale && img.rescale();
        });
    }
    ...
    if (localStorage.getItem("comicwidth")) {
        changeReaderWidth(localStorage.getItem("comicwidth"));
    }
    var d = document.createElement("div");
    d.id = "widthchanger";
    d.setAttribute("style", "height:20px;background-color: transparent;border-radius: 6px;padding: 0;text-align: center;overflow: hidden;font-size: 12px;margin: 4px auto;");
    d.setAttribute("class", "container flex");
    d.innerHTML = `<div class="container flex" style="height:20px;background-color: transparent;border-radius: 6px;padding: 0;text-align: center;overflow: hidden;font-size: 12px;margin: 4px auto;">
		<div onclick="changeReaderWidth(this)" class="bg-light closer" style="margin:0 1px;flex:1;width: 100%">100%</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-xl-block closer" style="margin:0 1px;width:93px;">1040</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-lg-block closer" style="margin:0 1px;width:93px;">850</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-md-block closer" style="margin:0 1px;width:88px;">660</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-sm-block closer" style="margin:0 1px;width:480px;">480</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-md-block closer" style="margin:0 1px;width:88px;">660</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-lg-block closer" style="margin:0 1px;width:93px;">850</div>
		<div onclick="changeReaderWidth(this)" class="bg-light d-none d-xl-block closer" style="margin:0 1px;width:93px;">1040</div>
		<div onclick="changeReaderWidth(this)" class="bg-light closer" style="margin:0 1px;flex:1;width: 100%">100%</div>
	</div>`;
    var pos = q(".tm-reader-top-nav")[0];
    pos.parentElement.insertBefore(d, pos);
    st.create(".closer:hover{background-color: #aaaaaa !important;}.modal{z-index:99999990;}").use();
    setTimeout(function () {
        return;
        window.nb.style.zIndex = "9999999";
        g(contentcontainer).addEventListener("click", function (ev) {
            var t = ev.target;
            if (t.tagName == "I") {
                window.pr(t);
                var bound = t.getBoundingClientRect();
                window.nb.style.top = bound.y + (window.scrollY || document.body.scrollTop) + bound.height + 5;
            }
        });
        namew.setValue = namew.__lookupSetter__("value");
        namew.getValue = namew.__lookupGetter__("value");
        namew.__defineSetter__("value", function (v) {
            namew.setValue(v);
            if (window.nameChangeTimeout) {
                clearTimeout(window.nameChangeTimeout);
            }
            window.nameChangeTimeout = setTimeout(function () {
                translator.onNameUpdate();
            }, 500);
        });
        namew.__defineGetter__("value", function () {
            return namew.getValue();
        });
        HTMLElement.prototype.__defineGetter__("cn", function () {
            return this.getAttribute("t");
        })
    }, 1000);
}

function changeReaderWidth(e) {
    var w = "100%";
    if (e.tagName)
        w = e.textContent.trim();
    else w = e || w;
    localStorage.setItem("comicwidth", w);
    if (w != "100%") {
        w += "px";
    }
    q(".tm-reader-top-nav,#content-container").forEach(function (ct) {
        ct.style.width = w;
        ct.style.margin = "auto";
    });
    window.onresize();
}

function onCaptchaSuccess(token, p) {
    ajax("ajax=verifycaptcha&token=" + encodeURIComponent(token) + "&purpose=" + p, function (res) {
        if (res == "success") {
            if (p == "register") {
                register();
            } else if (p == "login") {
...
ui.showAsGallery = function (img) {
    var click = function () {
        var ele = this;
        var d = document.createElement("div");
        d.setAttribute("style", "width: 100%; height: 100%; position: fixed; top: 0; left: 0; z-index: 999999");
        d.innerHTML = `<div class="top-box" style="height: 50px;width: 100%;position: absolute;left: 0;top: 0;z-index: 1;background: #00000026; opacity: 0; transition: opacity .2s;">
			<div style="text-align: right;">
				<button class="btn-close btn text-white" style="line-height: 40px;box-sizing: border-box;padding: 0 10px 10px 10px;margin: 0;border: none;font-size: 50px;">&times;</button>
			</div>
		</div><div class="im-box" style="height: 100%; overflow-y: auto"></div>`;
        var tImp = this.cloneNode(true);
        var bound = this.getBoundingClientRect();
        tImp.setAttribute("style", "position: absolute; width: " + bound.width + "; height: " + bound.height + "; left: " + bound.x + "; top: " + bound.y + "; transition: all .5s ease;");
        var imBox = d.querySelector(".im-box");
        var topBox = d.querySelector(".top-box");
        var btnClose = d.querySelector(".btn-close");
        var changeDisplayStyle = function () {
            if (tImp.style.position == "static") {
                tImp.style.position = 'absolute';
                tImp.style.height = "100%";
            } else {
                tImp.style.position = 'static';
                tImp.style.height = "auto";
            }
        };
        btnClose.addEventListener("click", function () {
            if (tImp.style.position == "static") {
                tImp.style.position = "absolute";
                tImp.style.height = tImp.clientHeight + "px";
            }
            requestAnimationFrame(function () {
                topBox.style.opacity = "0";
                d.style.background = "transparent";
                bound = ele.getBoundingClientRect();
                tImp.style.width = bound.width + "px";
                tImp.style.height = bound.height + "px";
                tImp.style.left = bound.x + "px";
                tImp.style.top = bound.y + "px";
                tImp.addEventListener("transitionend", function () {
                    d.remove();
                });
            });
        });
        imBox.appendChild(tImp);
        document.body.appendChild(d);
        requestAnimationFrame(function () {
            tImp.style.width = "100%";
            tImp.style.height = "100%";
            tImp.style.left = "0px";
            tImp.style.top = "0px";
            tImp.addEventListener("transitionend", transitionend);
            tImp.addEventListener("click", changeDisplayStyle);
        });
        var transitionend = function () {
            topBox.style.opacity = "1";
            d.style.background = "#333";
            tImp.removeEventListener("transitionend", transitionend);
        };
    };
    img.addEventListener("click", click);
}

ui.radio = function (opts, type = "select") {
    var d = document.createElement("div");
    d.setAttribute("class", "radio " + type);
    if (type == "select") {
        d.innerHTML = "<div class='radio-title'>" + opts.title + "</div><div class='radio-content'><div class='radio-bg-slider'></div></div>";
        var container = d.querySelector(".radio-content");
        var slider = d.querySelector(".radio-bg-slider");
        var options = opts.options;
        var optionWidth = (100 / options.length) + "%";
        for (var i = 0; i < options.length; i++) {
            var o = options[i];
            var r = document.createElement("div");
            r.setAttribute("class", "radio-option");
            r.textContent = o.text;
            r.value = o.value;
            r.setAttribute("value", o.value);
            r.style.width = optionWidth;
        ...
        d.addEventListener("click", function (e) {
            if (e.target.classList.contains("radio-option")) {
                ...
                requestAnimationFrame(function () {
                    ...
                    slider.style.left = selected.offsetLeft + "px";
                    slider.style.top = selected.offsetTop + "px";
    ...
    if (type == "checkbox") {
        d.innerHTML = "<div class='radio-title'>" + opts.title + "</div>";
        var options = opts.options;
        for (var i = 0; i < options.length; i++) {
            var o = options[i];
            var r = document.createElement("div");
...
function showNovelTranslatorOption() {
    var win = ui.win.create("Cài đặt dịch truyện");
    var body = win.body();
    var div = document.createElement("div");
    body.appendChild(div);
    var onTransEnableChange = null;
    var onTransLangChange = null;
    var onTransEngineChange = null;
    var currentConfig = {
        enable: "on",
        lang: "vi",
        engine: "convert"
    }
    var initialConfig = currentConfig;
    var parseCurrentConfig = function () {
        // compatible with old version
        var c1 = getCookie("transmode");
        var lg = getCookie("foreignlang");
        if (lg == "vi") {
            if (!c1 || ["name", "noname", "morehan"].indexOf(c1) != -1) {
                currentConfig = {
                    enable: "on",
                    lang: "vi",
                    engine: "convert"
                }
            } else if (["tfms", "tfms2"].indexOf(c1) != -1) {
                currentConfig = {
                    enable: "on",
                    lang: "vi",
                    engine: "bing"
        ...
        initialConfig = $.extend({}, currentConfig);
    }
    ...
    var applyConfig = function () {
        if (currentConfig.enable == "off") {
            setCookie("transmode", "chinese");
            setCookie("foreignlang", "vi");
            return;
        }
        if (currentConfig.engine == "convert") {
            setCookie("transmode", "name");
            setCookie("foreignlang", "vi");
            return;
        }
        if (currentConfig.engine == "bing") {
            if (currentConfig.lang == "vi") {
                setCookie("transmode", "tfms");
            } else {
                setCookie("transmode", "name");
            }
            setCookie("foreignlang", currentConfig.lang);
        }
        if (currentConfig.engine == "google") {
            setCookie("transmode", "name");
            setCookie("foreignlang", "gg_" + currentConfig.lang);
        }
        if (currentConfig.engine == "llm") {
            if (currentConfig.lang == "vi") {
                setCookie("transmode", "llm");
                setCookie("foreignlang", "vi");
            } else {
                setCookie("transmode", "name");
                setCookie("foreignlang", "llm_" + currentConfig.lang);
            }
        }
    }
    var clickCurrent = function (e, value) {
        if (!e) {
            return;
        }
        var op = e.querySelector(".radio-option[value='" + value + "']");
        if (op) {
            op.click();
        } else {
            e.querySelector(".radio-option").click();
        }
    }
    div.appendChild(ui.radio({
        title: "Bật dịch chương truyện",
        options: [{
            text: "Bật",
            value: "on"
        },
        ...
        onSelect: function (selected) {
            if (onTransEnableChange) {
                onTransEnableChange(selected);
            }
        }
    }));
    var transLang = ui.radio({
        ...
        onSelect: function (selected) {
            if (onTransLangChange) {
                onTransLangChange(selected);
            }
        }
    });
    div.appendChild(transLang);
    onTransEnableChange = function (selected) {
        transLang.style.display = selected == "on" ? "block" : "none";
        currentConfig.enable = selected;
        clickCurrent(transLang, currentConfig.lang);
        applyConfig();
        if (selected == "off") {
            if (transEngine) {
                transEngine.remove();
    ...
    onTransLangChange = function (selected) {
        if (transEngine) {
            transEngine.remove();
        }
        var opts = [{
        ...
        if (selected == "vi") {
            ...
            if (currentConfig.lang != "vi") {
                currentConfig.engine = "convert";
            }
        }
        transEngine = ui.radio({
            title: "Chọn công cụ dịch",
            options: opts,
            onSelect: function (selected) {
                if (onTransEngineChange) {
                    onTransEngineChange(selected);
                }
            }
        });
        div.appendChild(transEngine);
        currentConfig.lang = selected;
        clickCurrent(transEngine, currentConfig.engine);
        applyConfig();
    }
    onTransEngineChange = function (selected) {
        currentConfig.engine = selected;
        applyConfig();
    }
    parseCurrentConfig();
    onTransEnableChange(currentConfig.enable);
    var div2 = document.createElement("div");
    div2.innerHTML = `<p style="padding: 8px">Dùng dịch bằng AI sẽ cần cấu hình api, <a href="javascript:openLlmConfig()">Click vào đây</a> để cấu hình, bạn sẽ cần phải đăng nhập để cấu hình.</p>`;
    body.appendChild(div2);
    // click
    win.show();
    clickCurrent(body, currentConfig.enable);
    win._onclose = win.onclose;
    win.onclose = function () {
        if (
            currentConfig.enable != initialConfig.enable ||
            currentConfig.lang != initialConfig.lang ||
            currentConfig.engine != initialConfig.engine
        ) {
            location.reload();
        }
        win._onclose();
...
function POSTEventSource(url, options) {
    var self = this;
    options = options || {};
    this.url = url;
    ...
    this.onStatusCode = {
        add: function (code, listener) {
            if (!this[code]) {
                this[code] = [];
            }
            this[code].push(listener);
        },
        remove: function (code, listener) {
            if (this[code]) {
                var index = this[code].indexOf(listener);
                if (index != -1) {
    ...
    this.xhr.onprogress = function () {
        var newText = self.xhr.responseText.slice(previousLength);
        previousLength = self.xhr.responseText.length;
        self.buffer += newText;
        self.processBuffer();
    };
    ...
    this.xhr.onload = function () {
        ...
        if (self.onload) {
            var eventName =
                self.xhr.status >= 200 && self.xhr.status < 300 ?
                    "load" :
                    "error";
            self.onload(new Event(eventName));
        }
    };
    ...
    this.xhr.onloadend = function () {
        ...
        if (eventName == "error" && self.onerror) {
            self.onerror(new Event(eventName));
        }
        if (self.onclose) {
            if (self.onclose) self.onclose(new Event("close"));
        }
...
POSTEventSource.prototype.processBuffer = function () {
    var pos = 0;
    var lines = [];
    while (pos < this.buffer.length) {
        var crPos = this.buffer.indexOf("\r", pos);
        var lfPos = this.buffer.indexOf("\n", pos);
        var lineEnd;

        if (crPos !== -1 && (lfPos === -1 || crPos < lfPos)) {
            lineEnd = this.buffer[crPos + 1] === "\n" ? crPos + 2 : crPos + 1;
        } else if (lfPos !== -1) {
            lineEnd = lfPos + 1;
        } else {
            break;
        ...
        var line = this.buffer.slice(pos, lineEnd).replace(/\r\n|\n|\r$/, "");
        pos = lineEnd;
        lines.push(line);

        if (line === "") {
            this.processEvent(lines.slice(0, -1));
            lines.length = 0;
        }
    }
...
POSTEventSource.prototype.processEvent = function (lines) {
...
POSTEventSource.prototype.addEventListener = function (type, listener) {
    this["on" + type] = listener;
    if (type && type.match(/^\d+$/)) {
        this.onStatusCode.add(type, listener);
    }
};