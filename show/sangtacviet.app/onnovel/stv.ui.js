...
ui.zoomText = function(e, subSelector = null, saveScaleId = null, forceAllowZoom = false){
    ...
    if(!isZoomable || forceAllowZoom){
        ...
        var touch1, touch2, distance, scale, lastScale = 1, initialFontSize = parseFloat(window.getComputedStyle(getGetSetSizeElement()).fontSize);
...
ui.dragOutScroll = function(e,c){
	...
	var isCancel = false, isSwipeTimeout = false;
	var timer = null;
	var initDrag = false, isScrollEnd = true;
	var isTouchEnd = false, onScrollEnd = null;
	...
	var backToCenter = function(){
		...
		gsap.to(e.parentElement, {scrollTo: {x: vw, autoKill: true}, duration: 0.2});
	};
	var toDirection = function(dir, cb){
		...
		if(dir){ // ltr
			...
			gsap.to(e.parentElement, {scrollTo: {x: 0, autoKill: false}, duration: dur, onComplete: cb});
		}else{
			var dur = 0.25 * Math.abs((vw * 2 - left) / vw);
			gsap.to(e.parentElement, {scrollTo: {x: document.body.scrollWidth * 2, autoKill: false}, duration: dur, onComplete: cb});
		}
	};
	...
	e.parentElement.addEventListener("touchend", function(ev){
		initDrag = false;
		isTouchEnd = true;
		var cx = ev.changedTouches[0].screenX, cy = ev.changedTouches[0].screenY;
		co = cx - sx;
...
function loadComic(i){
	...
	setTimeout(function(){
		...
		HTMLElement.prototype.__defineGetter__("cn", function(){return this.getAttribute("t");})
	}, 1000);
}
function changeReaderWidth(e){
	...
	q(".tm-reader-top-nav,#content-container").forEach(function(ct){ct.style.width = w;ct.style.margin = "auto";});
	window.onresize();
}
...
function showNovelTranslatorOption(){
	...
	onTransLangChange = function(selected){
		...
		if(selected == "vi") {
			opts.unshift({text: "Convert", value: "convert"});
			if(currentConfig.lang != "vi"){
